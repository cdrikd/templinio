...

{
    ...
    data : 'url of json'
    container : 'id of the DIV which will embed timeline'
    timeline : {
        width: width of the svg timeline dflt 400
        height: height of the svg timeline dflt 300
    }
}
 */
...
Templinio = function(options) {
    ...
    options.timeline = options.timeline || {};  // TODO améliorer cela
    var w = options.timeline.width || 400;
    var h = options.timeline.height || 300;
    ...
    var paddingTimelineTop = 0; // Padding du haut
    if (typeof this.title != 'undefined') {
        // Si on a un titre on laisse la place pour l'écrire
        paddingTimelineTop = 50
    }
    console.log(paddingTimelineTop);
    var paddingXaxisBottom = 60; // Padding du bas pour l'axe X

    // Nombre de lignes de dates possibles . Hauteur - axe - (padding haut et bas)
    //var nbLine = Math.floor((h - paddingXaxisBottom - (2 * paddingData)) / (sizeLine + paddingData));
    ...
    function generateTimeline(dataset) {
        ...
        if (typeof this.title != 'undefined') {
            newsvg.append("text")
                    .attr("x",w/2)
                    .attr("y", paddingTimelineTop/2 )
                    .text(this.title)
                    .attr("class","templinioTitle");
        }

        // Create a rectangle for all timeline to use mousse wheel to zoom
        newsvg.append("rect")
            .attr("width", w)
            .attr("fill", "#fff")
            .attr("height", h - paddingXaxisBottom - paddingTimelineTop)
            .attr("transform","translate(0," + paddingTimelineTop + ")");

        newsvg.append("g")
            .attr("class", "x axis")
            .attr("transform","translate(0," + (h - paddingXaxisBottom) + ")")
            .call(xAxis);
        ...
        function zoomed(d) {
            div.style("opacity", 0);
            svg.select(".x.axis").call(xAxis); // Mise à jour de l'axe
            ...
            newsvg.selectAll("g.tevent.tdate").attr("transform", function(d) {
                var tr = d3.transform(d3.select(this).attr("transform")).translate;
                // On verifie si y n'est pas inférieur au paddingTimelineTop dans le cas de la premiere initialisation
                var newY = tr[1];
                if (newY < paddingTimelineTop) {
                    newY = paddingTimelineTop;
                }
                return "translate(" + (tScale(new Date(d.startDate)) - circleRadius) + "," + newY + ")";
            });
            newsvg.selectAll("g.tevent.tperiod").attr("transform", function(d) {
                ...
                return "translate(" + tScale(new Date(d.startDate)) + "," + newY + ")";
            });
            ...
            if (zoom.scale() != lastZoom) {
                events.each(function(d,i) {
                    ...
                    if (d3.select(this).classed("eventReduce")) {
                        ...
                        // l'évaluer
                        coordEvent.minY = paddingTimelineTop;
                        coordEvent.maxY = paddingTimelineTop + rectHeight;
                    }
                    ...
                    if (checkOverlap(parsed,coordEvent)) {
                        ...
                        while(coordEvent.maxY <= (h - paddingXaxisBottom - paddingData)) {
                            if (! checkOverlap(parsed,coordEvent)) {
                        ...
                        if (! collisionCorrected) {
                            coordEvent.minY = h-paddingXaxisBottom;
                            coordEvent.maxY = coordEvent.minY + rectHeight;
                            minimizeEvent = true;
                        } else {
                            if (d3.select(this).classed("eventReduce")) {
